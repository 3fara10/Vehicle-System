...
class SortTokenMasina(Command):
    def __init__(self, masina_service: MasinaService):
        self.__masina_service = masina_service

    def execute(self):
        print_masina(
            self.__masina_service.sort_alg_type(self.__masina_service.get_all_cars(), generic_comparator('_tokenMasina'), quicksort))

    def get_help(self):
        return "Sort the cars by token"
